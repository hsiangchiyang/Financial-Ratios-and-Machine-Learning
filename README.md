...
1. Create the single layer NN model with the package ‘neural net’ (https://www.rdocumentation.org/packages/neuralnet/versions/1.44.2/topics/neuralnet)
2. Instead of using all 13 ratios (generated from Process Data) directly as feature variables for training and generating only one model, we decide to observe the difference in models generated by the combinations of randomly picking three out of 13 ratios. This results in 286 models in total. 
3. As an attempt to add time dimension as part of feature variables, the new approach is to select annual ratios from all three years prior to the target year and flatten them into a new observation. For example, if  ‘current ratio,’ ‘debt ratio,’ and ‘roa ratio’ are selected as the feature variables, then there would be ‘current ratio 1’ ‘current ratio 2’ ‘current ratio 3’ ‘debt ratio 1’ ‘debt ratio 2’ ‘debt ratio 3’ ‘roa ratio 1’ ‘roa ratio 2’ ‘roa ratio 3’ instead of only one value column for each ratio.
4. The Target label is set to be the Binary labels (0/1), showing whether the upcoming stock price is higher than the average of all the previous years. 
5. Randomly split the data from Process Data into 70% training and 30% testing.
6. Since the number of feature variables for each model is nine (3 ratios* 3 years), the number of hidden neurons is currently set to six as it is ⅔ of nine. 

7. After executing the file model.R, we have generated 286 models, and the resulting data is saved as the file results.RData.  In analysis.R, after having all the models loaded, we firstly use every model to generate the accuracies by comparing its predictions with actual labels on the testing datasets, then find the top ten models that have the best performance based on accuracy.
...
## Pre-Generated Files to Run Analysis.R:

https://drive.google.com/drive/folders/1Tj6QJi3tGK9SecXIPuJ7FDEUtiZIg9mZ?usp=sharing
